...
#             WTConv2d(input_channel, input_channel) for _ in range(5)
#         ])
#         self.leaky_relus = nn.ModuleList([
#             nn.LeakyReLU(negative_slope=0.1, inplace=True) for _ in range(5)
#         ])
...
#         # target_size = (1, 48, 480, 640)
#         # if image_feat.size(2) < target_size[2] or image_feat.size(3) < target_size[3]:
#         #     padding = (
#         #         0,
#         #         target_size[3] - image_feat.size(3),
#         #         0,
#         #         target_size[2] - image_feat.size(2)
#         #     )
#         #     image_feat = F.pad(image_feat, padding, mode='replicate')
...
#         # print("fused_feat shape :",image_feat.shape )
...
#         # print(attn_feat.shape,fused_feat.shape,'aaaaaaaaaaaaaaaaaaaasssssssssssssssssssssssssssssssssssssss')
#         resized_feat = (fused_feat * attn_feat).view(original_shape[0], C, H, W)
#         # resized_feat = (fused_feat * attn_feat).view(original_shape[0], C, 96, 96)
#         # print(resized_feat.shape,'ssssssssssssssssssssssssss')
...
#         # print(text.shape,'ssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssss')
#         text_feat = self.text_process(text)
#         # print(text_feat.shape,'ssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssss')
...
#         #     padding = (
#         #         0,
#         #         target_size[3] - L_fused.size(3),
#         #         0,
#         #         target_size[2] - L_fused.size(2)
#         #     )
#         #     L_fused = F.pad(L_fused, padding, mode='replicate')
...
#         # target_size = (1, 48, 480, 640)
#         # if R_fused.size(2) < target_size[2] or R_fused.size(3) < target_size[3]:
#         #     padding = (
#         #         0,
#         #         target_size[3] - R_fused.size(3),
#         #         0,
#         #         target_size[2] - R_fused.size(2)
#         #     )
#         #     R_fused = F.pad(R_fused, padding, mode='replicate')
#         R_fused = self.imagef2textf(R_fused)
#         R_fused = R_fused + self.image_process(ClipImage)
#         # print(R_fused.shape, 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa')
...
#         key = key.transpose(0, 1)
#         value = value.transpose(0, 1)
#         # print(query.shape, key.shape, value.shape ,'sssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssss')
#         attn_output, _ = self.multihead_attn(query, key, value)

#         attn_output = attn_output.transpose(0, 1)

#         # 去除 seq_len 维度，得到 [batch_size, 64]
...
#         # print(out_enc_level1.shape,'-000-0-0--0-')
...
#         # print(inp_dec_level3.shape,inp_enc_level4.shape )
...
#         # for layer in self.refinement_1:
#         #     out_dec_level1 = layer(out_dec_level1, [H, W])
...
#         # out_dec_level1 = self.decoder_final_1(out_dec_level1)
#         # out_dec_level1 = self.decoder_final_2(out_dec_level1)
#         # out_dec_level1 = self.decoder_final_3(out_dec_level1)
...
#     imageB = torch.randn(batch_size, 1, 16, 16).to('cuda:1')
#     ClipImage = torch.randn(batch_size, 512).to('cuda:1')
#     clip_text = torch.randn(batch_size, 512).to('cuda:1')
...
#     output = model(imageB, imageA, clip_text, blip_text,ClipImage)
#     print(output.shape)
...
#     target = torch.randn(1, 3, 16, 16).to('cuda:1')
#     imageA = torch.randn(batch_size, 3, 16, 16).to('cuda:1')
#     imageB = torch.randn(batch_size, 1, 16, 16).to('cuda:1')
#     ClipImage = torch.randn(batch_size, 512).to('cuda:1')
#     clip_text = torch.randn(batch_size, 512).to('cuda:1')
...
#     #             # 如果输出是列表，检查每个元素
#     #             shapes = [o.shape if isinstance(o, torch.Tensor) else str(type(o)) for o in output]
#     #             print(f"{name}: {shapes}")
...
#     # # 前向传播
#     # with torch.no_grad():
#     #     output = model(imageB, imageA, clip_text, blip_text)

#     # # 计算损失（在评估模式下这步通常不需要，除非特别需要评估损失值）
#     # loss = loss_fn(output, target)


# # if __name__ == "__main__":
# #     batch_size = 2
# #     imageA = torch.randn(batch_size, 3, 64,64).to('cuda')
# #     imageB = torch.randn(batch_size, 1, 64, 64).to('cuda')
# #     ClipImage = torch.randn(batch_size, 512).to('cuda')
# #     clip_text = torch.randn(batch_size, 512).to('cuda')
# #     blip_text = torch.randn(batch_size, 149, 768).to('cuda')
...
# #     target = torch.randn(1, 3, 32, 32).to('cuda')
# #     imageA = torch.randn(batch_size, 3, 32,32).to('cuda')
# #     imageB = torch.randn(batch_size, 1, 32, 32).to('cuda')
# #     ClipImage = torch.randn(batch_size, 512).to('cuda')
# #     clip_text = torch.randn(batch_size, 512).to('cuda')
# #     blip_text = torch.randn(batch_size, 149, 768).to('cuda')
